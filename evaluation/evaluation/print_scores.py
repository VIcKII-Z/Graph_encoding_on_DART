...
if __name__ == '__main__':
    ...
    with open('parent.txt') as f:
        parent = float(f.read().strip().split()[-1])
    print("PARENT: {:.2f}".format(parent))
    
    print(' & '.join(["{:.2f}".format(bleu), "{:.2f}".format(meteor), "{:.2f}".format(ter), "{:.2f}".format(moverscore), "{:.2f}".format(bertscore), "{:.2f}".format(bleurt), "{:.2f}".format(parent)]))
